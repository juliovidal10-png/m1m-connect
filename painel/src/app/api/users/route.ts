...
export async function GET() {
  try {
    ...
    console.error("ERRO USERS GET:", error);

    return NextResponse.json(
      {
        error: getErrorMessage(error, "Erro ao carregar os usuários."),
      },
      {
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("ERRO USERS POST:", error);

    return NextResponse.json(
      {
        error: getErrorMessage(error, "Erro ao criar o usuário."),
      },
      {
        status: getErrorStatus(error),
      },
