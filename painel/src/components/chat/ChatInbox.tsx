...
function mergeDuplicateChats(items: Chat[]) {
  ...
  for (const chat of items) {
    ...
    /*
       * Regra de unicidade da lista:
       * - conversa individual: uma identidade WhatsApp canonica = uma linha;
       * - grupo: o JID do grupo continua sendo a identidade.
       *
       * crmCustomerId permanece como dado associado ao CRM/atendimento,
       * mas nao define a identidade visual da conversa.
       */
      const chatIdentity = isGroupChat(chat)
        ? `jid:${chat.remoteJid}`
        : `jid:${canonicalJid}`;
